...
impl ParseTable {
        ...
        for (state, items) in collection.collection.iter().enumerate() {
            for item in items {
                if item.is_end(grammar) {
                    table.add_reductions(grammar, state, item)?;
    ...
    fn add_reductions(&mut self, grammar: &Grammar, from: usize, item: &LRItem) -> Result<()> {
        ...
        let i = match item.lookahead {
            InputSymbol::Character(c) => grammar.terminal_index(c),
        ...
        self.actions[from][i] = if grammar.production(item.production).head == grammar.start() {
            TableEntry::Accept
        } else {
...
impl Parser {
    /// Creates a new canonical LR parser
    ...
    pub fn parse(&self, input: &str) -> Result<Tree> {
        ...
        loop {
            ...
            match self.table.action(stack.last().unwrap().state, lookahead) {
    ...
    fn reduce(&self, id: usize, stack: &mut Vec<StackEntry>, tree: &mut Tree) {
        ...
        stack.push(StackEntry {
            state: if let TableEntry::Goto(next) = self
                .table
                .action(stack.last().unwrap().state, production.head)
            {
                next
            } else {
...
mod test {
    ...
    fn test_parse_table() -> std::result::Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(
            table.actions[0][g.terminal_index('c')],
            TableEntry::Shift(3)
        );
        assert_eq!(
            table.actions[0][g.terminal_index('d')],
            TableEntry::Shift(4)
        );
        ...
        assert_eq!(table.actions[1][g.terminal_index('c')], TableEntry::Error,);
        assert_eq!(table.actions[1][g.terminal_index('d')], TableEntry::Error,);
        ...
        assert_eq!(
            table.actions[2][g.terminal_index('c')],
            TableEntry::Shift(6)
        );
        assert_eq!(
            table.actions[2][g.terminal_index('d')],
            TableEntry::Shift(7)
        );
        ...
        assert_eq!(
            table.actions[3][g.terminal_index('c')],
            TableEntry::Shift(3)
        );
        assert_eq!(
            table.actions[3][g.terminal_index('d')],
            TableEntry::Shift(4)
        );
        ...
        assert_eq!(
            table.actions[4][g.terminal_index('c')],
            TableEntry::Reduce(3)
        );
        assert_eq!(
            table.actions[4][g.terminal_index('d')],
            TableEntry::Reduce(3)
        );
        ...
        assert_eq!(table.actions[5][g.terminal_index('c')], TableEntry::Error);
        assert_eq!(table.actions[5][g.terminal_index('d')], TableEntry::Error);
        ...
        assert_eq!(
            table.actions[6][g.terminal_index('c')],
            TableEntry::Shift(6)
        );
        assert_eq!(
            table.actions[6][g.terminal_index('d')],
            TableEntry::Shift(7)
        );
        ...
        assert_eq!(table.actions[7][g.terminal_index('c')], TableEntry::Error);
        assert_eq!(table.actions[7][g.terminal_index('d')], TableEntry::Error);
        ...
        assert_eq!(
            table.actions[8][g.terminal_index('c')],
            TableEntry::Reduce(2)
        );
        assert_eq!(
            table.actions[8][g.terminal_index('d')],
            TableEntry::Reduce(2)
        );
        ...
        assert_eq!(table.actions[9][g.terminal_index('c')], TableEntry::Error);
        assert_eq!(table.actions[9][g.terminal_index('d')], TableEntry::Error);
